...
vec4 Crayon2(vec2 vTexCoor, float xDistance, float yDistance) {
    ...
    vec3 s00 = texture(sTexture, pos + vec2(-xDistance, yDistance)).rgb;
    vec3 s10 = texture(sTexture, pos + vec2(xDistance, yDistance)).rgb;
    vec3 s20 = texture(sTexture, pos + vec2(-xDistance, -yDistance)).rgb;
    vec3 s01 = texture(sTexture, pos + vec2(xDistance, -yDistance)).rgb;

    vec3 color = texture(sTexture, pos).rgb;
    vec3 maxcolor;
    vec3 color_T;
...
void main() {
    ...
    uFragColor = Crayon2(vTexCoor, xDistance, yDistance);
}
